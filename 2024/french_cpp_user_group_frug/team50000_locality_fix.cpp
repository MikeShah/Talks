// @file team50000_locality_fix.cpp
// g++ -std=c++23 team50000_locality_fix.cpp -o prog -lpthread
#include <iostream>
...
void helper(){
	std::cout << std::thread::hardware_concurrency() << " # of concurrent threads supported.\n";
}
...
int main() {
		...
		for(int j=0; j < 50'000; j++){
				...
				for(int i=0; i < 16; i++){
						threads.push_back(std::jthread(AdditionWorker,i,16));
				} 
		}
